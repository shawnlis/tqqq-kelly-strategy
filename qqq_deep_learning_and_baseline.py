...
def train_deep_learning_model(df, features, look_ahead=5, prev_thresholds=None, ema_alpha=0.2):
    ...
    if not np.isfinite(q_low) or not np.isfinite(q_high) or q_high <= q_low:
        q_low, q_high = float(q_low_inst), float(q_high_inst)
    targets = pd.cut(score, bins=[-np.inf, q_low, q_high, np.inf],
    ...
    class_weight = {int(k): float(total / (3.0 * vc.get(k, 1.0))) for k in [0, 1, 2]}
    ...
    weight_tensor = torch.tensor(
        [class_weight.get(0, 1.0), class_weight.get(1, 1.0), class_weight.get(2, 1.0)],
        dtype=torch.float32,
        device=device
    )
...
def compute_effective_leverage_from_weights(weights_df):
    """
    Estimate current beta exposure relative to QQQ using the latest weights snapshot.
    Treat futures as L_base * w_fut, TQQQ as 3x, and QQQ5 as 5x.
    Returns a tuple of (asof_timestamp, effective_leverage).
    """
...
def baseline_backtest(df,
                      ...
                      rebalance_every_days=5,    # 周期调仓（5个交易日）
    ...
    for i in range(1, len(idx)):
        ...
        if (i - last_rebal_day) >= rebalance_every_days and i >= 1:
            ...
            if policy_mode == 'bandit':
                try:
                    ...
                    pass
            # 应用
            kelly_frac_live = decision["kelly_frac"]
        ...
        def book_trade(frm,to,frac):
            ...
            if moved>0:
                cost = ((trade_cost_bps + slip_bps)/10000.0) * moved * new_eq
                try:
                    if adv_series is not None:
                        adv_today = adv_series.asof(d) if hasattr(adv_series, "asof") else None
                        if adv_today is None and hasattr(adv_series, "reindex"):
                            adv_today = adv_series.reindex([d]).ffill().bfill().iloc[0]
                        if adv_today is not None:
                            adv_val = float(adv_today)
                            if np.isfinite(adv_val) and adv_val > 0:
                                dollar_trade = moved * new_eq
                                impact_frac = impact_k * (dollar_trade / adv_val)
...
def deep_learning_backtest(df,
    ...
    if DL_AVAILABLE:
        ...
        init = train_deep_learning_model(train_df, train_feat, prev_thresholds=None)
        if init:
    ...
    for i in range(1, len(idx)):
        ...
        if (i - last_rebal_day) >= rebalance_every_days and i >= 1:
            ...
            if DL_AVAILABLE and i >= 252 and (i - last_retrain_day) >= rebalance_every_days:
                ...
                upd = train_deep_learning_model(rec_df, rec_feat, prev_thresholds=prev_th)
                if upd:
        ...
        if model is not None and scaler is not None and predict_fn is not None and dl_stats is not None:
            try:
                ...
                dl_log = (
                    ...
                    f"exp_score={expected_score:.2f}, kf={kelly_frac_live:.2f}, L={L_base:.2f}, cooldown={risk_cooldown_left})"
                )
                if action in (1, 2):
...
def main():
    ...
    ap.add_argument('--kelly_frac', type=float, default=0.7)
    ap.add_argument('--kelly_lookback_days', type=int, default=252*3)
    ap.add_argument('--rebal_days', type=int, default=5)  # weekly-ish
    ...
    ap.add_argument('--bandit_alpha', type=float, default=0.6)
    ...
    ap.add_argument('--target_vol', type=float, default=0.35, help='Annualized target vol cap for QQQ exposure')
    ...
    if not args.start or not args.end:
        ...
        today = dt.date.today(); end = today
        start = dt.date(today.year-10, today.month, min(today.day, 28))
        args.start = args.start or start.isoformat()
        args.end = args.end or end.isoformat()
        print(f"Using default period: {args.start} to {args.end}")
    ...
    common_kw = dict(
        ...
        dl_max_qqq5=args.dl_max_qqq5, dl_max_tqqq=args.dl_max_tqqq, dl_trade_max_frac=args.dl_trade_max_frac,
        dl_cooldown=args.dl_cooldown, adv_series=adv_series, slip_bps=args.slip_bps, impact_k=args.impact_k
    )
